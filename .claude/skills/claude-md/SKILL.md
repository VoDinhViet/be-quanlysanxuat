...
argument-hint: "[create|update|audit] [path]"
---
...
## This project's own baseline (audit run 2026-07-24)

Root `CLAUDE.md` (84 lines) was audited against the Validation Checklist above and found compliant: under target length, no style rules, no business-logic code snippets (the `## Commands` bash block is the "Development Commands" essential section, not an anti-pattern), progressive disclosure already in place (`@.claude/rules/*.md` imports match the `agent_docs/` pattern above), and the `## Notes` section serves as "Known Issues / Gotchas". Use this as the baseline for the next `audit` run — flag genuinely *new* drift (e.g. the dated module-history paragraphs in `## Project` growing past the point of being "surprising, not obvious"), not the same already-reviewed content.
