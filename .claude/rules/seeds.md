...
Reference: `credentials.seed.ts` (multi-entity curated — the only seed left in this template).
...
- MUST make every seed idempotent — safe to run repeatedly, never duplicating data.
  - **Curated data** (roles, departments, positions, credentials, and any future lookup table):
    MUST look up each row by its unique key (`code`, `username`, ...) with `findFirst` before
    inserting, and skip + log if it exists. MUST NOT blind-insert.
  - **Bulk fake data** (`drizzle-seed`): no example in this template yet. `seed()` has no per-row
    idempotency, so MUST gate the whole call behind a single `findFirst` on a known key and skip
    entirely if it already ran.
