...
- Modules that were deliberately removed: `orders`, `uploads` (superseded by `files`). Don't search for them or recreate them unless explicitly asked. `suppliers`/`supplier-groups` were removed and then rolled back on 2026-07-20 — they **do** exist. So do `clients`, `products`, `roles` and the RBAC schema — see `CLAUDE.md` for the current module list.
