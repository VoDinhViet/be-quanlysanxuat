...
- MUST NOT treat `@Permissions()` as enforced on an `@ApiPublic()` route — both guards skip public routes, so the decorator is inert there. No route in this template stacks them today (keep it that way); an `@ApiPublic()` route is **fully unauthenticated**. MUST NOT "secure" one by adding a permission — remove `@ApiPublic()` instead, and confirm with the user first since it breaks existing callers.
