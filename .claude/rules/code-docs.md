...
Reference implementation: `ProductionOrderStatus`/`ProductionJobStatus` class-level docs and
`ProductionOrdersService.seedPlan` (`src/database/schemas/production.ts`,
`src/api/production-orders/production-orders.service.ts`).
...
2. **Summary + `Rules:`** — once a symbol carries two or more independent constraints: one summary sentence, a blank `*` line, a `Rules:` line, one bullet per constraint:
...
- Viết bằng **tiếng Việt** — tên symbol/identifier trong dấu backtick vẫn giữ nguyên tiếng Anh (khớp code thật), chỉ phần diễn giải là tiếng Việt.
