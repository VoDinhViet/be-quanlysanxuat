...
| `units`                | product-structure | Đơn vị tính (ĐVT), CRUD đầy đủ; `scope` (`PRODUCT`/`MATERIAL`/`SEMI_FINISHED`) quyết định dùng được ở đâu — `PATCH` gửi `scopes` thì xoá + chèn lại toàn bộ, cùng khuôn `clients` contacts                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `inventory`            | inventory         | chỉ đọc — `/inventory/balances` mặc định lọc FG/RM (kho không quản tồn WIP, `docs/decisions/wip-not-stocked.md`, gửi tường minh `itemType=WIP` vẫn xem được), `/inventory/transactions` không đổi (mọi loại); sở hữu `InventoryPostingService`; **không còn route list** — đã tách sang `inventory-products`/`inventory-materials`                                                                                                                                                                                                                                                                                                            |
| `inventory-products`   | inventory         | Tồn kho thành phẩm — `GET /inventory-products` (cứng FG) + `GET /inventory-products/:itemId/ledger` (thẻ kho — sổ cái 1 item, tồn luỹ kế `balanceAfter`, "loại giao dịch"/"diễn giải" cố ý để FE tự suy, không trả sẵn)                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `inventory-materials`  | inventory         | Tồn kho vật tư — `GET /inventory-materials` (cứng RM); chưa có thẻ kho riêng, đợt sau                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `inventory-requisitions` | inventory       | Phiếu lãnh vật tư — chứng từ **duy nhất** đưa RM ra khỏi kho cho sản xuất (`POST /inventory-issues` với `issueType=PRODUCTION` bị chặn, `E234`); vòng đời riêng 6 trạng thái `DRAFT`/`PENDING_APPROVAL`/`APPROVED`/`ISSUED`/`REJECTED`/`CANCELLED`; `type=PRODUCTION` bắt buộc `productionJobId` (`E233`), `type=OTHER` dùng `reason`; giữ chỗ ("Đã giữ") bắt đầu từ `approve`, không phải `create`; `issue` (`APPROVED → ISSUED`, điểm cuối) tự sinh 1 `inventory_issues` `POSTED` + trừ tồn; `GET .../lines` popup chọn vật tư dùng chung "+ Lãnh từ LSX"/"+ Lãnh khác"; xem `docs/workflows/inventory-requisition.md`               |
...
| `qc-aql`               | quality           | Master data phương án lấy mẫu AQL (`qc_aql_plans`/`qc_aql_rules`) — thay hardcode `LOT_SIZE_CODE_LETTER`/`SAMPLING_PLAN` cũ, `docs/decisions/qc-aql-master-data.md`; `GET`/`POST /qc-aql/plans`, `PATCH /qc-aql/plans/:planId` (gửi `rules` thì xoá + chèn lại toàn bộ, cùng khuôn `clients` contacts), chưa có `DELETE`; `iqc`/`oqc` đọc qua `resolveAqlPlan()` (`src/api/iqc/iqc-aql.query.ts`)                                                                                                                                                                                                                                             |
| `iqc`                  | quality           | Kiểm tra chất lượng hàng nhập — `qc_requests` (lô kiểm, 1 dòng/vật tư) + `qc_inspections` (mỗi lần "Lưu" = 1 attempt append-only, không đè, `docs/decisions/qc-request-attempt-split.md`); `GET` list/`GET stats`/`GET .../aql-plan` (gợi ý cỡ mẫu, mirror `oqc`)/detail, `POST` tạo, `POST :id/confirm` (nút "Lưu" duy nhất, QC tự chọn `result`), `PATCH :id` (sửa ngữ cảnh sau confirm), `DELETE :id` (chỉ khi `NOT_INSPECTED`, `E206`) — đường gỡ đi kèm gate `inventory-issues` mới; `status` (trên `qc_requests`, mirror attempt mới nhất) suy từ `result`/`disposition`                                                                                                                                                                                                                                                                                |
...
| `reports`              | —                 | Chỉ `GET /reports/stats` — số liệu tổng hợp dashboard, hạ tầng thuần, không thuộc domain nghiệp vụ nào                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
