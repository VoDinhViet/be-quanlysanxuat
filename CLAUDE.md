...
| `outbound-orders` | inventory | Phiếu giao hàng (DO) — header + nhiều dòng (`outbound_order_items`); `POST` tạo nháp (luôn `DRAFT`) + `GET` list/detail + `GET .../items` + `POST :id/confirm` (`DRAFT → PENDING_DELIVERY`, chặn `E205` nếu còn Job nào chưa qua hết OQC — `getJobOqcClearance`), chưa có duyệt/`PATCH`/`DELETE`/xác nhận giao thật, chưa đụng tồn kho, chưa giữ chỗ (reserve); mỗi dòng trỏ 1 `order_item` (không unique — giao nhiều lần/1 PO), client gửi thẳng `itemId`/`productionJobId` (lấy từ popup), server không resolve/validate lại; `GET .../unfulfilled-order-items` popup chọn PO/Job, listing thuần theo trạng thái đơn (không tính tồn/giữ chỗ) |
