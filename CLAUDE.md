...
| `outsourcing-orders` | inventory | Phiếu gửi gia công ngoài (OS-OUT) — header + nhiều dòng (`outsourcing_order_items`), **không có nháp** — `POST` tạo là `POSTED` ngay, trừ tồn kho gửi luôn trong cùng transaction (`docs/decisions/outsourcing-no-draft.md`); mỗi dòng bắt buộc gắn `productionJobOperationId` của một Job đang `IN_PROGRESS`, công đoạn snapshot `type = OUTSOURCE`, chặn gửi vượt định mức Job; chỉ còn `cancel`, không có `PATCH`/`DELETE`/`post` riêng; `GET .../outsourceable-operations` popup chọn part; import `InventoryModule`+`WarehousesModule` |
| `outsourcing-receipts` | inventory | Phiếu nhận gia công ngoài (OS-IN) — header + nhiều dòng (`outsourcing_receipt_items`), cũng không có nháp — `POST` tạo là `POSTED` ngay, cộng tồn kho nhận luôn, sinh N phiếu IQC nếu `requiresIqc`; mỗi dòng trỏ đúng 1 dòng OS-OUT, **1 phiếu OS-IN = 1 NCC** nhưng gộp được nhiều OS-OUT khác nhau cùng NCC; chỉ còn `cancel`, không có `PATCH`/`DELETE`/`post` riêng; `GET .../pending-order-items` popup chọn hàng cần nhận; import `InventoryModule`+`WarehousesModule`+`IqcModule` |
