...
| ------ | ------------------------------------------------ | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| GET    | `/production-orders`                             | `production:read`    | `GetProductionOrdersReqDto` — `limit`, `page`, `q` (khớp mờ mã PO/mã LSX), `status` (`PENDING`/`APPROVED`), `clientId`, `fromDate`/`toDate` (theo `dueDate`)              | `200` + `ProductionOrderResDto` phân trang — **màn hình chính LSX**, 1 dòng/PO, mọi LSX đã sinh (kể cả PO đã `COMPLETED`/`CANCELLED`)                              |
...
| GET    | `/production-jobs`                               | `production:read`    | `GetProductionJobsReqDto` — `limit`, `page`, `q` (khớp mờ mã Job/mã LSX/mã PO/mã-tên SP), `orderId`, `productId`, `status`, `clientId`, `fromDate`/`toDate` (theo `dueDate` của PO) | `200` + `ProductionJobResDto` phân trang — **"Quản lý sản xuất"**, 1 dòng/Job, xem "Response list vs detail"                                          |
| GET    | `/production-jobs/:jobId`                        | `production:read`    | —                                                                                                                                                                         | `200` + `ProductionJobDetailResDto`                                                                                                                                |
| POST   | `/production-jobs/:jobId/start`                  | `production:update`  | —                                                                                                                                                                         | `200` + `ProductionJobDetailResDto` — `PENDING → IN_PROGRESS`                                                                                                      |
| POST   | `/production-jobs/:jobId/report`                 | `production:update`  | `ReportProductionJobReqDto` — `producedQty?`, `rejectedQty?`                                                                                                              | `200` + `ProductionJobDetailResDto` — cộng dồn `producedQty`/`rejectedQty`                                                                                          |
| POST   | `/production-jobs/:jobId/hold`                   | `production:update`  | —                                                                                                                                                                          | `200` + `ProductionJobDetailResDto` — `IN_PROGRESS → WAITING`                                                                                                      |
| POST   | `/production-jobs/:jobId/resume`                 | `production:update`  | —                                                                                                                                                                         | `200` + `ProductionJobDetailResDto` — `WAITING → IN_PROGRESS`                                                                                                      |
