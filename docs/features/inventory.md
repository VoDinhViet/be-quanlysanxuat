...
**Module LSX (`production-orders`) hiện chưa tự sinh phiếu `OUT`/`DELIVERY`** — duyệt LSX (`docs/features/production.md`) chỉ chốt số lượng sản xuất và sinh Job, chưa lập phiếu xuất kho cho "Lấy từ tồn" (xem "Ngoài phạm vi" của `production.md`). Cách nối duy nhất giữa hai module hiện tại là `InventoryService.getStockLevels`/`reservedSubquery` nhận tham số tuỳ chọn `excludeOrderId`, cho `ProductionOrdersService` tính "Khả dụng" của một PO mà không tự trừ nhu cầu của chính nó — `GET /inventory` không dùng tham số này. Phiếu `OUT`/`DELIVERY` gắn `orderItemId` vẫn chỉ lập được tay qua `POST /stock-receipts`.
