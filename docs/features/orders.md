...
| `totalValue`                   | Tổng giá trị               | `sum(total * exchangeRate)`                                                                                                                                                                       |
| `totalValueTrendPercent`       | % so với tháng trước       | như trên, theo `sum(total * exchangeRate)`; `null` nếu tháng trước = 0                                                                                                                            |
| `completedValue`               | Đã giao                    | `sum(total * exchangeRate)` của các đơn `status = COMPLETED`                                                                                                                                      |
