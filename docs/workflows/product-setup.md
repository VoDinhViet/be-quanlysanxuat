...
Người dùng khai báo một item mới (FG hoặc DIRECT), hoặc tạo biến thể từ một FG đã có. Thứ tự bắt buộc:
...
- Thêm node BOM: item gốc tồn tại và không phải DIRECT (`E111`). Hai nhánh theo `type` — Cấp 0
  ...
  - `DIRECT` — bắt buộc `itemId` trỏ đúng item `type = DIRECT` (không phải FG); sai → `E270`.
  Node cha (nếu có) phải cùng cây và không phải lá DIRECT (`E052`); thêm `DIRECT` thì node cha
  (kể cả Cấp 0 khi không gửi `parentId`) chưa được có con `COMPONENT` (`E273`); `quantity` phải nguyên
  dương nếu node là `COMPONENT`, được phép lẻ nếu là `DIRECT` (`E055`).
- Thêm công đoạn cho node `COMPONENT`: node phải thuộc đúng item trên URL (`E051`); node đó không
  được là `DIRECT` (`E063`) — DIRECT là lá, không có công đoạn as-used.
- Thêm công đoạn Cấp 0: item gốc không được là DIRECT (`E111`).
...
3. **Dựng cây.** `parentId = NULL` trên một node `bom_items` nghĩa là "ngay dưới Cấp 0" (không phải
   ...
   khi ghi. Một node `COMPONENT` có thể có con; một node `DIRECT` luôn là lá. **Dựng cấu trúc
...
4. **Gắn công đoạn — cùng khuôn as-used, hai route tách theo cấp:** công đoạn của một node
   ...
   Không có ràng buộc thứ tự đặc biệt cho vật tư nữa — DIRECT chỉ là một node bình thường trong
   `POST .../bom/items`, không cần một bước khai riêng.
5. **Tạo biến thể (tuỳ chọn, chỉ FG).** `POST /items/:itemId/copy` (`E110` nếu DIRECT), body nhận
   ...
   mới; node `DIRECT` giữ nguyên `itemId`, node `COMPONENT` copy nguyên `code`/`name`) + clone
...
- Thêm node `COMPONENT` vào một node đang có lá DIRECT **xoá ngầm toàn bộ DIRECT cùng cha**
  trong cùng transaction, không cảnh báo.
- Xoá một node giữa cây **cascade sạch cả nhánh con (kể cả lá DIRECT) và công đoạn as-used
...
- Nhân bản clone **cấu trúc cây + công đoạn as-used (`bom_operations`) của mọi node COMPONENT +
  `routing_operations` của Cấp 0 + `item_files`**: các item DIRECT được node lá tham chiếu giữ
...
| Tình huống | Mã |
| --- | --- |
| Item gốc của BOM/routing là DIRECT | `E111` |
| Node DIRECT trỏ tới item không phải DIRECT (kể cả FG) | `E270` |
| Node COMPONENT sai shape (thiếu `code`/`name`, hoặc kèm `itemId`) | `E271` |
| Node cha là lá DIRECT (không nhận con) | `E052` |
| Thêm DIRECT vào node đã có con COMPONENT | `E273` |
| SL node COMPONENT không nguyên | `E055` |
| Node không thuộc item trên URL — công đoạn | `E051` |
| Gắn công đoạn vào node DIRECT | `E063` |
| Nhân bản một item DIRECT | `E110` |
| Dòng công đoạn (`PATCH`/`DELETE`) không tồn tại | `E109` (mọi cấp, kể cả Cấp 0) |

Node anh em trùng nhau hợp lệ, trừ khi cả hai là DIRECT cùng `itemId` (`E245`). Xem
`docs/domains/product-structure.md`.

## Business rules

- Vì sao routing thuộc **vị trí** chứ không thuộc item, vì sao DIRECT luôn là lá, vì sao node COMPONENT
