 ...
 * (joined with `product`/`material`, each narrowed to `code`/`name` only) — Drizzle's relational
 * query type inference struggles here (same class of issue as `RawBomItemRow` in
 * `boms.service.ts`), so the query result is cast to this at the call site. */
