 ...
 * Drizzle sẽ thiếu `url` (ảnh 404 âm thầm) và lộ `storageKey`/`checksum`/`uploadedBy`.
 * Property dạng `files` (mảng đính kèm) không cần hàm này vì không có `@Transform` riêng, `@Type`
 * tự convert. */
