 ...
 * này toàn cục, không scope được theo `warehouseId`. */
export function outboundHeldQuantityByItemSubquery(db: Database) {
  return db
    .select({
    ...
    .where(inArray(outboundOrders.status, HOLDING_STATUSES))
