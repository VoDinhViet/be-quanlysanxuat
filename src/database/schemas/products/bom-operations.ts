...
/**
 * Công đoạn as-used gắn thẳng vào một node `bom_items` cụ thể — cùng khuôn `bom_materials`, tách ra
 * từ `routing_steps` (nay chỉ còn giữ ca Cấp 0). Không còn XOR `productId`/`bomItemId`: mọi dòng
 * bắt buộc có `bomItemId`.
 *
 * Rules:
 * - `operationId` bất biến sau khi thêm — đổi công đoạn là xoá dòng rồi thêm lại, cùng quy ước
 *   `bom_materials.materialId`.
