 ...
 * - `Database` is deliberately **not** assignable to it (drizzle's `PgTransaction` carries
 *   protected members), so handing a helper the pooled connection by mistake is a compile error
 *   rather than a silent escape onto a separate connection that commits on its own.
 */
